...
Supported settings include `host`, `port`, `aof.enabled`, `aof.path`, `aof.fsync`, `snapshot.path`, `snapshot.interval.seconds`, `log.level`, `limits.max.value.bytes`, `limits.max.array.elements`, `limits.max.connections`, optional `auth.password`, `limits.client.idle.timeout.seconds`, and `limits.max.commands.per.second`. For containers, prefer the `MYREDIS_AUTH_PASSWORD` environment variable so the password is not stored in an image configuration file. A limit of `0` disables command rate limiting.
